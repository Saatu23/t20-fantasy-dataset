...
base_folder = ""
json_folder = os.path.join(base_folder, "match_25")
output_file = os.path.join(base_folder, "2025_filtered_ipl_training_data.csv")
...
for player in player_stats:
    ...
    if matches > 0:
        ...
        stats["Total current fantasy points"] = (matches*4 + stats["Runs"]+(stats["Fours"]*4)+(stats["Sixes"]*6)+(stats["25+ Scores"]*4)+(stats["50+ Scores"]*8)+(stats["75+ Scores"]*12)+(stats["100+ Scores"]*16)
        ...
        stats["Current Fantasy points per match"] = round(stats["Total current fantasy points"]/matches, 2)
